...
1. **零硬编码地址**：LE 地址全动态发现，不写死任何 MAC
2. **协议自动识别**：按 GATT 服务指纹路由，不依赖型号名
3. **坐标集中配置**：弹窗布局参数收敛到 PopupProfile，不散落魔法数字
4. **跨进程解耦**：App 与 GMS 通过广播通信，互不直接持有引用
5. **自愈闭环**：地址丢失 → 扫描 → 发现 → 连接 → 缓存 → 秒连
