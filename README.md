...
- **3.0**：设备常驻通知合并为一条（电量 + 降噪档位按钮，大图标 + M3 动态取色，按设备能力动态生成档位）；修复官方降噪面板点击无反应（点击走管理器发送出口，原 hook 的两条路径都到不了，且发包被「等上一次 SET 响应」永久挡住）；设置页新增官方集成 / 通知四类功能开关，并按功能域重组为外观 / 功能 / 自定义映射 / 后台 / 诊断。
